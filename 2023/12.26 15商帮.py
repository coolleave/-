...
def handle_novel_mc_1():
    ...
    with open(f'{name}.txt', 'r', encoding='utf-8') as file:
        txt = file.read()
        txt = re.sub(r".*?年.*?月.*?日", '【】', txt)
    with open(f'{name}1.txt', 'w', encoding='utf-8') as w:
        w.write(txt)
        print('文本标准化处理完毕！')
    # # 打开Excel文件
    ...
    # 打开小说文本文件
    with open(f'{name}1.txt', 'r', encoding='utf-8') as file:
        print('分配章节')
    ...
    for line in lines:
        ...
        if line.startswith('【】'):
            ...
            if chapter_title and chapter_content:
                ...
                #
                worksheet.cell(row=chapter_index+index, column=1, value=chapter_content)
