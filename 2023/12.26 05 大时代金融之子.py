...
def handle_novel_mc_1():
    ...
    # 处理txt文件
    with open('大时代之金融之子1.txt', 'r', encoding='utf-8') as file:
        txt = file.read()
        txt = re.sub(r".*?ps：.*?", '【】', txt)
        # txt = re.sub(r".*?未完待续*?", '【】', txt)
    with open('大时代之金融之子1.txt', 'w', encoding='utf-8') as w:
        w.write(txt)
        print('文本标准化处理完毕！')
    # # 打开Excel文件
    ...
    for line in lines:
        ...
        if line.startswith('【】'):
            ...
            if chapter_title and chapter_content:
                ...
                worksheet.cell(row=chapter_index+2225, column=1, value=chapter_content)
