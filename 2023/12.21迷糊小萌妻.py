...
def handle_novel_mc_1():
    ...
    # 处理txt文件
    with open('迷糊小萌妻.txt', 'r', encoding='utf-8') as file:
        pattern1 = re.compile(r'第\d{3}章.*?')
        txt = file.read()
        blocks = re.findall(pattern1, txt)
        for block in blocks:
            txt = txt.replace(block, '【】')
    with open('迷糊小萌妻1.txt', 'w', encoding='utf-8') as w:
        w.write(txt)
        print('文本标准化处理完毕！')
    # 打开Excel文件
    ...
    # 打开小说文本文件
    with open('迷糊小萌妻1.txt', 'r', encoding='utf-8') as file:
        print('分配章节')
