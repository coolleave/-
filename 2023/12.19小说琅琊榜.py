...
def handle_novel_mc_1():
    ...
    # 处理txt文件
    with open('琅琊榜.txt', 'r', encoding='utf-8') as file:
        pattern1 = re.compile(f'.*?分割线.*?。')
        txt = file.read()
        blocks = re.findall(pattern1, txt)
        for block in blocks:
            txt = txt.replace(block, '【】')
    with open('琅琊榜1.txt', 'w', encoding='utf-8') as w:
        w.write(txt)
        print('文本标准化处理完毕！')
    # 打开Excel文件
    ...
    # 打开小说文本文件
    with open('琅琊榜1.txt', 'r', encoding='utf-8') as file:
        print('分配章节')
